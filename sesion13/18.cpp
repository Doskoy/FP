...
public:
   ...
   //La funcion no funciona correctamente.
   void EliminaExcesoBlancos(){
      SecuenciaCaracteres copia_texto; 
      copia_texto.Aniade(vector_privado[0]);
      for(int i = 1; i < total_utilizados; i++){
         if(copia_texto.Elemento(i-1) == vector_privado[i] && vector_privado[i] != ' '){
            i++;
         }
         else{
            copia_texto.Aniade(vector_privado[i]);
      ...
      for(int i = 0; i < copia_texto.TotalUtilizados(); i++){
         vector_privado[i] = copia_texto.Elemento(i);
         total_utilizados++; 
      }
   }
